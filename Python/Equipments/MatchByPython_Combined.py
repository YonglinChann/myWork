...
# 使用两种清理方法进行匹配
data_df['Matched_Title'] = data_df['Cleaned_Title'].apply(
    lambda x: get_best_match(x, choices, lookup_dict)
)

data_df['Matched_Device'] = data_df['Cleaned_Device'].apply(
    lambda x: get_best_match(x, choices, lookup_dict)
)

# 计算匹配分数
data_df['Title_Score'] = data_df.apply(
    lambda row: fuzz.token_sort_ratio(str(row['Cleaned_Title']).lower(), str(row['Matched_Title']).lower()) 
    if pd.notna(row['Matched_Title']) else 0, 
    axis=1
)

data_df['Device_Score'] = data_df.apply(
    lambda row: fuzz.token_sort_ratio(str(row['Cleaned_Device']).lower(), str(row['Matched_Device']).lower()) 
    if pd.notna(row['Matched_Device']) else 0, 
    axis=1
)
...
result_columns = ['pro_title', 'pro_brand', 'pro_model', '机型数据', 
                 'Cleaned_Title', 'Cleaned_Device', 'Final_Match', 'Match_Score', 'ID']
result_df = result_df[result_columns]
result_df['ID'] = result_df['ID'].fillna("未找到")

# 保存到新的Excel文件
result_df.to_excel("Result_Combined.xlsx", index=False)
print("匹配完成。输出已保存到 'Result_Combined.xlsx'。")
