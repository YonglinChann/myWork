...
padded_image = np.pad(dithered_image,
                      ((kernel_size // 2, kernel_size // 2), (kernel_size // 2, kernel_size // 2), (0, 0)),
                      mode='symmetric')  # 使用symmetric模式以更好地保持边缘
...
mapped_image = cv2.resize(mapped_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
dithered_image = cv2.resize(dithered_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
