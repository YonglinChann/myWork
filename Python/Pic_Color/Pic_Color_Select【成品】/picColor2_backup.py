...
# 使用OpenCV进行缩放，使用INTER_AREA算法以获得更好的缩小效果
dithered_image = cv2.resize(dithered_image, (new_width, new_height), interpolation=cv2.INTER_AREA)
...
plt.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'scaled_image.png'), dpi=300, bbox_inches='tight')
plt.show()

# 裁剪图像到200x200像素
print("\n需要将缩放后的图像裁剪为200x200像素")
print(f"缩放后尺寸: {w}x{h}像素")
...
if use_center == 'y' or use_center == '':
    ...
    # 以左下角为原点进行裁剪
    print(f"图片尺寸为{w}x{h}，需要裁剪为200x200")
    
    start_x = 0
    start_y = 0
    user_y_display = 0 # 用于最后打印信息

    # 仅当宽度大于200时，才需要询问x坐标
    if w > 200:
        print("请指定裁剪区域的左侧起始x坐标（左下角为原点）")
        while True:
            try:
        ...
        print("宽度已为200，x坐标自动设为0")

    # 仅当高度大于200时，才需要询问y坐标
    if h > 200:
        print("请指定裁剪区域的底部起始y坐标（左下角为原点）")
        while True:
            try:
                # 用户输入的是以左下角为原点的y值
                ...
                if 0 <= user_y <= h-200:
                    ...
                    user_y_display = user_y # 保存用户输入的y值用于显示
                    break
                print(f"y坐标应在0到{h-200}之间")
            except ValueError:
                print("请输入有效的整数")
    else:
        print("高度已为200，y坐标自动设为0")
        # start_y 保持为 0
        user_y_display = 0

    print(f"将从左下角原点坐标({start_x}, {user_y_display})开始裁剪200x200区域")
...
def find_closest_color(pixel):
    ...
    distances = [np.sqrt(np.sum((pixel_array - target_color) ** 2)) for target_color in target_colors.values()]
    closest_idx = np.argmin(distances)
