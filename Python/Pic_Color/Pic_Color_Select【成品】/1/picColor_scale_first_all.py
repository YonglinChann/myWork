...
def process_image_for_eink_default(input_path, output_image_path, output_packets_path=""):
    ...
    try:
        ...
        for i, color in enumerate(np.array(palette, dtype=np.float32)):
            # 计算与每个目标颜色的欧氏距离
            # 计算与每个目标颜色的欧氏距离
            distances = {}
            ...
            closest_color = target_colors.get(closest_color_name, target_colors['黑色'])
        ...
        for y in range(h):
            for x in range(w):
                ...
                # 使用对应颜色的阈值判断是否足够接近
                if min_distance < color_thresholds.get(closest_color_name, float('inf')):
                ...
                if pixel not in rgb_to_color_name:
                    ...
                    for name, target_color in target_colors.items():
                        ...
                        if distance < min_distance:
                            ...
                            closest_color = name
                            # 直接修正像素值为目标颜色
                            cropped_image[y, x] = target_colors.get(closest_color, target_colors['黑色'])
                    color_name = closest_color
                else:
                    color_name = rgb_to_color_name.get(tuple(map(int, pixel)), '黑色')
                binary_pixels.append(color_to_binary.get(color_name, color_to_binary.get('黑色', '00')))
...
def process_hex_data_default(file_path: str) -> List[str]:
    """
    处理default算法生成的十六进制数据文件 (JSON格式)，提取数据包信息
    
    参数:
        file_path (str): 十六进制数据文件路径 (应为JSON数组格式)
        
    返回:
        list: 提取的十六进制数据包列表
    """
    if not file_path or not os.path.exists(file_path):
        print(f"错误：default数据文件不存在或路径无效: {file_path}")
        return []
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                ...
                if all(isinstance(item, str) for item in data):
                    print(f"成功从 {file_path} 读取 {len(data)} 个数据包 (default)")
                    return data
                else:
                    print(f"错误: {file_path} 中的JSON数组包含非字符串元素 (default)")
                    return []
            else:
                print(f"错误: {file_path} 的内容不是一个有效的JSON数组 (default)")
                return []
    except json.JSONDecodeError as e:
        print(f"错误: 解析JSON文件 {file_path} 失败 (default): {str(e)}")
        return []
    except Exception as e:
        print(f"处理default数据文件 {file_path} 时发生未知错误: {str(e)}")
        return []
...
def process_hex_data_T(file_path: str) -> List[str]:
    """
    处理T算法生成的十六进制数据文件，提取数据包信息
    ...
    """
    if not file_path or not os.path.exists(file_path):
        print(f"文件不存在或路径无效 (T): {file_path}")
        return []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if not content.startswith('[') or not content.endswith(']'):
                print(f"文件格式错误 (T): {file_path} 未找到JSON数组结构")
                return []
            content = content.strip('[]').strip()
            if not content: # 处理空数组的情况 '[]'
                return []
            # 按逗号分割并处理每个数据包
            ...
            for packet in packets:
                ...
                if clean_packet:  # 确保不是空字符串
                    hex_packets_T.append(clean_packet)
    except Exception as e:
        print(f"处理T数据文件 {file_path} 时出错: {str(e)}")
        return []
...
def process_hex_data_T(file_path: str) -> List[str]:
    """
    处理T算法生成的十六进制数据文件 (JSON格式)，提取数据包信息
    ...
    """
    if not file_path or not os.path.exists(file_path):
        print(f"错误：T数据文件不存在或路径无效: {file_path}")
        return []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                if all(isinstance(item, str) for item in data):
                    print(f"成功从 {file_path} 读取 {len(data)} 个数据包 (T)")
                    return data
                else:
                    print(f"错误: {file_path} 中的JSON数组包含非字符串元素 (T)")
                    return []
            else:
                print(f"错误: {file_path} 的内容不是一个有效的JSON数组 (T)")
                return []
    except json.JSONDecodeError as e:
        print(f"错误: 解析JSON文件 {file_path} 失败 (T): {str(e)}")
        return []
    except Exception as e:
        print(f"处理T数据文件 {file_path} 时发生未知错误: {str(e)}")
        return []


def process_hex_data_C(file_path: str) -> List[str]:
    """
    处理C算法生成的十六进制数据文件，提取数据包信息
    ...
    """
    if not file_path or not os.path.exists(file_path):
        print(f"文件不存在或路径无效 (C): {file_path}")
        return []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            content = f.read().strip()
            if not content.startswith('[') or not content.endswith(']'):
                print(f"文件格式错误 (C): {file_path} 未找到JSON数组结构")
                return []
            content = content.strip('[]').strip()
            if not content: # 处理空数组的情况 '[]'
                return []
            ...
            for packet in packets:
                ...
                clean_packet = packet.strip().strip('"').strip()
                if clean_packet:  # 确保不是空字符串
                    hex_packets_C.append(clean_packet)
    except Exception as e:
        print(f"处理C数据文件 {file_path} 时出错: {str(e)}")
        return []
...
def process_image_for_eink_C(input_path, output_image_path, output_packets_path=""):
    ...
    try:
        ...
        def map_blue_green_colors(pixel, target_colors):
            ...
            # 默认情况：使用欧氏距离找最近的标准颜色
            distances = {name: float(np.sqrt(np.sum((np.array(pixel, dtype=np.float32) - np.array(target_color, dtype=np.float32)) ** 2)))
                        for name, target_color in target_colors.items()}
            closest_color_name = min(distances.items(), key=lambda x: x[1])[0]
            return closest_color_name, target_colors.get(closest_color_name, target_colors['黑色'])
...
def process_hex_data_C(file_path: str) -> List[str]:
    """
    处理C算法生成的十六进制数据文件 (JSON格式)，提取数据包信息
    ...
    """
    if not file_path or not os.path.exists(file_path):
        print(f"错误：C数据文件不存在或路径无效: {file_path}")
        return []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                if all(isinstance(item, str) for item in data):
                    print(f"成功从 {file_path} 读取 {len(data)} 个数据包 (C)")
                    return data
                else:
                    print(f"错误: {file_path} 中的JSON数组包含非字符串元素 (C)")
                    return []
            else:
                print(f"错误: {file_path} 的内容不是一个有效的JSON数组 (C)")
                return []
    except json.JSONDecodeError as e:
        print(f"错误: 解析JSON文件 {file_path} 失败 (C): {str(e)}")
        return []
    except Exception as e:
        print(f"处理C数据文件 {file_path} 时发生未知错误: {str(e)}")
        return []


def process_hex_data_light_red(file_path: str) -> List[str]:
    """
...
def process_hex_data_combined(file_paths_dict):
    ...
    返回:
        list: 包含算法类型、图片路径和对应二进制数据的字典列表
              格式如：[
                      {'style': 'default', 'src': '../nft-upload-file/2024-6/default.png', 'data': ['二进制数据1', '二进制数据2']},
                      {'style': 'T', 'src': '../nft-upload-file/2024-6/T.png', 'data': ['二进制数据1', '二进制数据2']},
                      {'style': 'C', 'src': '../nft-upload-file/2024-6/C.png', 'data': ['二进制数据1', '二进制数据2']},
                      {'style': 'light_red', 'src': '../nft-upload-file/2024-6/light_red.png', 'data': ['二进制数据1', '二进制数据2']}
                     ]
    """
    result_list = []
    ...
    if 'default' in file_paths_dict:
        for file_path in file_paths_dict['default']:
            if file_path:
                ...
                result_list.append({
                    'style': 'default',
                    'src': img_path.replace(os.path.dirname(img_dir), '../nft-upload-file'),
                    'data': hex_data
                })
...
def process_hex_data_light_red(file_path: str) -> List[str]:
    """
    处理light_red算法生成的十六进制数据文件 (JSON格式)，提取数据包信息
    ...
    """
    if not file_path or not os.path.exists(file_path):
        print(f"错误：light_red数据文件不存在或路径无效: {file_path}")
        return []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                if all(isinstance(item, str) for item in data):
                    print(f"成功从 {file_path} 读取 {len(data)} 个数据包 (light_red)")
                    return data
                else:
                    print(f"错误: {file_path} 中的JSON数组包含非字符串元素 (light_red)")
                    return []
            else:
                print(f"错误: {file_path} 的内容不是一个有效的JSON数组 (light_red)")
                return []
    except json.JSONDecodeError as e:
        print(f"错误: 解析JSON文件 {file_path} 失败 (light_red): {str(e)}")
        return []
    except Exception as e:
        print(f"处理light_red数据文件 {file_path} 时发生未知错误: {str(e)}")
        return []
...
def process_image_combined(image_name, target_dir):
    ...
    # 定义统一的数据输出文件路径
    output_data_file = f"{folder_path}/data.txt"
    print(f"输出数据文件路径: {output_data_file}")
    ...
    try:
        ...
        # 创建临时文件路径
        temp_data_files = {
            'default': f"{folder_path}/.temp_default_data.txt",
            'T': f"{folder_path}/.temp_T_data.txt",
            'C': f"{folder_path}/.temp_C_data.txt",
            'light_red': f"{folder_path}/.temp_light_red_data.txt"
        }
        
        print(f"\n----- 开始使用default算法处理图像 -----")
        # 使用default算法处理图像
        result_default = process_image_for_eink_default(input_image, output_image_default, temp_data_files['default'])
        print(f"default算法处理结果: {'成功' if result_default else '失败'}")
        
        print(f"\n----- 开始使用T算法处理图像 -----")
        # 使用T算法处理图像
        result_T = process_image_for_eink_T(input_image, output_image_T, temp_data_files['T'])
        print(f"T算法处理结果: {'成功' if result_T else '失败'}")
        
        print(f"\n----- 开始使用C算法处理图像 -----")
        # 使用C算法处理图像
        result_C = process_image_for_eink_C(input_image, output_image_C, temp_data_files['C'])
        print(f"C算法处理结果: {'成功' if result_C else '失败'}")
        
        print(f"\n----- 开始使用light_red算法处理图像 -----")
        # 使用light_red算法处理图像
        result_light_red = process_image_for_eink_light_red(input_image, output_image_light_red, temp_data_files['light_red'])
        ...
        if result_default and result_T and result_C and result_light_red:
            ...
            # 清理临时文件
            for temp_file in temp_data_files.values():
                try:
