...
def process_image_for_eink(input_path, output_image_path, output_packets_path=""):
    ...
    参数:
        ...
        output_packets_path (str): 输出二进制数据列表文件路径
    ...
    try:
        ...
        for i, color in enumerate(palette):
            # 计算与每个目标颜色的欧氏距离
            distances = {name: np.sqrt(np.sum((color - target_color) ** 2)) for name, target_color in
                         target_colors.items()}
        ...
        for y in range(h):
            for x in range(w):
                ...
                elif closest_color_name == '红色' and old_pixel[0] > red_intensity_threshold and old_pixel[0] > \
                        old_pixel[1] * red_ratio_threshold and old_pixel[0] > old_pixel[2] * red_ratio_threshold:
                    is_close_to_target = True
...
result = process_image_for_eink(
    input_path="/Users/chenyonglin/myCode/gitee/myWork/Python/Pic_Color/Pic_Color_Select【成品】/1/pic/2024-21.jpg",
    output_image_path="/Users/chenyonglin/myCode/gitee/myWork/Python/Pic_Color/Pic_Color_Select【成品】/1/pic/2024-21_processed.jpg"
)

if result:
    print("图像处理成功!")
    # print(f"处理后的图像已保存至: pic/2024-17_processed.jpg")
    # print(f"数据包已保存至: pic/2024-17_data.txt")
else:
    print("图像处理失败!")