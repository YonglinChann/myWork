...
if not photo_files:
    raise FileNotFoundError(f'未在photos文件夹中找到photo开头的图片文件，请确保在{photos_dir}中存在photo1.jpg或photo2.png等格式的图片')
...
# 对抖动处理后的图像应用拉普拉斯算子
edges = cv2.filter2D(dithered_image, -1, laplacian_kernel)

# 控制边缘增强的强度
enhancement_strength = 0.3
enhanced_dithered = cv2.addWeighted(dithered_image, 1, edges, enhancement_strength, 0)

# 确保像素值在有效范围内
enhanced_dithered = np.clip(enhanced_dithered, 0, 255).astype(np.uint8)

# 保存边缘增强后的图像
cv2.imwrite(os.path.join(output_dir, 'enhanced_dithered.png'), cv2.cvtColor(enhanced_dithered, cv2.COLOR_RGB2BGR))
...
plt.subplot(1, 3, 3)
plt.imshow(enhanced_dithered)
plt.title('边缘增强后的图像')
plt.axis('off')

plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'edge_enhancement_comparison.png'), dpi=300, bbox_inches='tight')
plt.show()

# 图像缩放处理
print("\n开始进行图像缩放处理...")
target_width = 128  # 设置目标宽度
scale_ratio = target_width / w
target_height = int(h * scale_ratio)

# 使用Lanczos插值算法进行缩放
scaled_original = cv2.resize(image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
scaled_mapped = cv2.resize(mapped_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
scaled_dithered = cv2.resize(dithered_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4)
...
plt.title('缩放后的直接映射图像')
plt.axis('off')

plt.subplot(1, 3, 3)
plt.imshow(scaled_dithered)
plt.title('缩放后的抖动处理图像')
plt.axis('off')
